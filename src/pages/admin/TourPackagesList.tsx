...
import { Plus, Edit2, Trash2, Search, Map, Image as ImageIcon } from 'lucide-react';
import { getTourPackages, deleteTourPackage } from '../../services/firebase/tourPackages';
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-900">Kelola Paket Wisata</h1>
        <Link
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 text-gray-600 text-sm font-medium">
              <tr>
                ...
                <th className="px-6 py-4">Layanan Termasuk</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                    Tidak ada paket wisata ditemukan.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={pkg.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="h-12 w-12 rounded-lg bg-gray-100 overflow-hidden flex items-center justify-center">
                          {pkg.images && pkg.images.length > 0 ? (
                            <img src={pkg.images[0]} alt={pkg.name} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                          ) : (
                        ...
                        <div>
                          ...
                          <span className="text-xs text-gray-500">{pkg.duration || 'Durasi tidak diatur'}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        {pkg.includedServices.slice(0, 3).map((service, idx) => (
                          <span key={idx} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[10px] font-medium rounded-full">
                            {service}
                          </span>
                        ))}
                        {pkg.includedServices.length > 3 && (
                          <span className="text-[10px] text-gray-400">+{pkg.includedServices.length - 3} lagi</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Link
                          to={`/admin/tour-packages/edit/${pkg.id}`}
